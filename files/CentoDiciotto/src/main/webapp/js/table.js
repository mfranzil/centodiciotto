...
$.getScript(getContextPath().toString() + "/js/popup.js", function () {
});
        ...
        $.each(headers, function (index1, header) {
            ...
            if (item.hasOwnProperty(header.field)) {
                switch (header.type) {
                    ...
                    case "photo" : {
                        tableRows += "<img class=\"avatar-small\" src=\"" + getContextPath().toString() + item[header.field] + "\" alt =\"Profile photo\">";
                        break;
                    }
                    case "button" : {
