    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(user instanceof GeneralPractitioner){
            try {
                ...
                List<WholePatient> whole_patient = new ArrayList<>();
                for(Patient patient: patientList){
                    String photoPath = PhotoService.getLastPhoto(patient);

                    whole_patient.add(new WholePatient(patient.getFirstName() + " " + patient.getLastName(), patient.getSsn(), photoPath, patient.getID(), "Patient Data"));
                }

                Gson gson = new Gson();

                response.setContentType("application/json");
                response.getWriter().write(gson.toJson(whole_patient));
